...
\indent To illustrate these quantities in an applied context, we will compute them on and example network shown in Figure \ref{fig:Cytometry}. This network is constructed from a single cell mass cytometry dataset released publicly and processed using the Cytofkit R package \cite{cytofkit}. Each node represents a single cell and is represented with 52 features for a mass cytometry analysis. Briefly, mass cytometry  We created a network from this dataset by selecting 500 cells and building a $k$-nearest neighbor network with $k=5$. This means that for a node $i$, we found its $5$ nearest neighbors according to Euclidean distance, and connected them all to node $i$. 
